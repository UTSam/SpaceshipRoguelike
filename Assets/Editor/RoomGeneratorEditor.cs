...
public class RoomGeneratorEditor : Editor
{
    ...
    private void saveTilemapAsAsset(RoomGenerator gen, string filename)
    {
        ...
        if (File.Exists(Application.dataPath + "/Scenes/Rooms/" + filename + ".prefab"))
        {
    ...
    {
        bool saved = PrefabUtility.SaveAsPrefabAsset(prefabToSave, "Assets/Scenes/Rooms/" + filename + ".prefab");
        if (saved)
        {
    ...
    private void drawEditingSection()
    {
        ...
        if (roomObject)
        {
            if (GUILayout.Button("Add to scene - DELETES ALL CURRENTLY DRAWN"))
            {
                ...
                tempRoonObject.SetTileMap(gen.tilemap);
